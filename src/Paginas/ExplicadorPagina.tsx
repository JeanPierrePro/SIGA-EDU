...
const ExplicadorPagina: React.FC = () => {
    ...
    const [sessoesNovas, setSessoesNovas] = useState(0);
    ...
    const handleAbrirAgenda = () => {
        setAbaAtiva('agenda');
        setSessoesNovas(0); // Limpa o badge ao abrir
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-indigo-700 to-slate-900 flex">
            ...
            <div className="w-64 bg-white/5 backdrop-blur-xl border-r border-white/10 text-white flex flex-col shadow-2xl shrink-0">
                ...
                <nav className="flex-1 p-4 mt-4 space-y-2">
                    <button
                        ...
                        className={`w-full flex items-center justify-between py-3 px-4 rounded-xl transition-all ${abaAtiva === 'agenda' ? 'bg-indigo-500/40 shadow-inner border border-white/20 text-white' : 'hover:bg-white/10 border border-transparent text-indigo-200 hover:text-white'}`}>
                        <div className="flex items-center space-x-3">
                            <CalendarDays className="w-5 h-5 opacity-90" />
                            <span className="font-bold text-sm tracking-wide">Minha Agenda</span>
                        </div>
                        {/* PONTO VERMELHO — aparece quando há sessões novas */}
                        {sessoesNovas > 0 && abaAtiva !== 'agenda' && (
                ...
                <div className="p-4 border-t border-white/10 bg-black/10 flex flex-col gap-2">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center space-x-2 bg-white/5 hover:bg-red-500/90 py-2.5 rounded-xl font-bold transition-all border border-white/10 text-indigo-100 hover:text-white group mt-1">
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <main className="flex-1 p-8 overflow-y-auto">
                    {abaAtiva === 'agenda'
                        ? <CalendarioExplicador onNovasSessoes={setSessoesNovas} />
                        : <PerfilExplicador />
                    }
                </main>
            </div>
